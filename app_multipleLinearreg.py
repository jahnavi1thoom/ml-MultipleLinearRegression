...
st.dataframe(data[["total_bill","size","tip"]].head(10))
...
#visualization
st.markdown('<div class="card">',unsafe_allow_html=True)
st.subheader("Total Bill vs Tip Amount(with multiple linear regression)")
fig, ax = plt.subplots()
...
size=st.slider("Group Size",int(data["size"].min()),int(data["size"].max()),2)
